...
aficion(marge, leer).
aficion(bart, skate). 
aficion(lisa, jugar_munhecas).
aficion(maggie, chupar_chupete).
aficion(abe, contar_historias). 
aficion(patty_bouvier, fumar).
aficion(selma_bouvier, fumar).
...
aficion(moe_szyslak, hablar_con_amigos).
aficion(ned_flanders, rezar).
aficion(rod_flanders, rezar). 
aficion(todd_flanders, rezar). 
aficion(edna_krabappel, ninhos). 
aficion(luann_van_houten, leer). 
aficion(kirk_van_houten, comer_palomitas). 
aficion(milhouse_van_houten, lisa). 
...
madre(luann_van_houten, milhouse_van_houten).
...
hermanos(bart, lisa).
hermanos(lisa, bart).
hermanos(lisa, maggie).
hermanos(maggie, lisa).
...
hijo_invitado(X) :- (amigo_de_homer(Y); amiga_de_marge(Y)), (padre(Y,X); madre(Y,X)), estudia(X, colegio_primaria), ninho(X).
...
comida(X, [costilla(1), criollo(1)]) :- come(X, churrasco), come(X, criollo), not(come(X, chorizo)).  % come churrasco y criollo
comida(X, [costilla(1), chorizo(1)]) :- come(X, churrasco), not(come(X, criollo)), come(X, chorizo).  % come churrasco y chorizo
comida(X, costilla(2)) :- come(X, churrasco), not(come(X, criollo)), not(come(X, chorizo)).  % come churrasco
comida(X, [criollo(1), chorizo(1)]) :- not(come(X, churrasco)), come(X, criollo), come(X, chorizo).  % come criollo y chorizo

bebida(X, cerveza) :- adulto(X), aficion(X, beber_bar_moe).
bebida(X, vino) :- adulto(X), not(bebida(X, cerveza)).
bebida(X, limonada) :- ninho(X).