...
% b. Se é pai de alguén, entón é adulto e é home. Igualmente, se é nai de
alguén, entón é adulto e é muller.
adulto(X) :- padre(_, X).
adulto(X) :- madre(_, X).
hombre(X) :- padre(_, X).
mujer(X) :- madre(_, X).

% c. Se dous personaxes distintos son fillos do mesmo pai e da mesma nai,
entón son irmáns.
hermanos(X, Y) :- padre(X, A), padre(Y, A), madre(X, B), madre(Y, B), X \= Y.

% d. Se dous personaxes distintos comparten pai ou nai, pero non ambos,
entón son medio irmáns.
...
come(X, krusty_burger) :- \+ es(X, vegetariano).
...
% c- As grande xerras de cervexa do bar de Moe teñen unha capacidade
de 2 litros. Calcular a cantidade de litros que bebe Homer se bebe N xerras, sendo N un parámetro de entrada, que debe ser sempre un número enteiro maior ou igual a 0.
litros_bebidos(N, L) :- capacidad_jarras(C), L is N * C.

% d- Sen tele e sen cervexa, Homer perde a cabeza.
pierde_cabeza(homer) :- \+ tiene(homer, cerveza), \+ tiene(homer, television).
