class MaquinaExpendedora:
    def __init__(self, archivo):
        ...
        self.cargar_automata(archivo)


    def leer_cabecera(self, linea):
        """
        Procesa las primeras líneas del archivo.
        """
    ...
    def clausura_lambda(self, estados_iniciales):
        ...
        continuar = True
        while continuar:
            nuevos_estados = estados_actuales[:]
            for estado in estados_actuales:
                nuevos_estados += self.funcion_transicion(estado, "λ")
            
            nuevos_estados = list(set(nuevos_estados))  # Eliminar duplicados
    ...
    def procesar_cadena(self, cadena):
        ...
        for simbolo in cadena:
            if simbolo not in self.alfabeto:
                print(f"Error: El símbolo '{simbolo}' no pertenece al alfabeto del autómata.")
                return False
            
            nuevos_estados = list()
            for estado in estados_actuales:
                nuevos_estados.extend(self.funcion_transicion(estado, simbolo))

            estados_actuales = self.clausura_lambda(nuevos_estados)
            print(f"Transición ({estado}, '{simbolo}') -> {estados_actuales}")
